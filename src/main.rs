...
fn extract_metrics(bamp: String, fraction: f32, _threads: usize) {
    ...
    let (mut lengths, mut pids): (Vec<u32>, Vec<f32>) = extract_from_bam::extract(&bamp, inverse);
    ...
    let bamname = bampath.file_name().unwrap().to_str().unwrap();
    let checksum = hash_file(bampath, checksums::Algorithm::MD5);
    println!("bam\tnumber_of_reads\tyield\tn50\tmedian_length\tmedian_pid\thistogram_length\thistogram_pid\tbam_path\tbam_checksum");
    println!(
        "{bamname}\t{numreads}\t{data_yield}\t{n50}\t{median_length}\t{median_pid}\t{bamp}\t{checksum}"
    )
}
